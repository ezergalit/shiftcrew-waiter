...
export const STOPS = {
  ...
  // 🔴 `exam_ready` (GDB, 16.9): המבחן המלא פתוח מהכניסה הראשונה, ולכן «אחרי מספיק תרגול נפתח
  // הבוחן» הבטיח את ההפך ממה שעל המסך. שם השורה אומרת מה שכן נכון; בלי הדגל — הנוסח של יותם.
  "learn-door": ({ examReady }) => (examReady
    ? "המבחן המלא כבר פתוח — אפשר להיבחן עכשיו, או לתרגל קודם קטגוריה בכרטיסיות."
    : "פה מתרגלים בעזרת כרטיסיות. אחרי מספיק תרגול נפתח הבוחן על אותו חלק בתפריט."),
