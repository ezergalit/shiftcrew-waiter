...
// ⚠️ נמדד מול שני התפריטים החיים (14.9): «גיוזה **פרגית**» ו«סיגר **לוקוס**» הוכרזו
// טבעוניות כי המילים לא היו כאן — עוף ודג שהוגשו ללקוח טבעוני. כל שם חלבון שמופיע
// בתפריט חייב להיות ברשימה; אחרי כל ייבוא לרוץ שוב על הפיקסצ׳רים.
const ANIMAL = ["דג", "דגים", "סלמון", "טונה", "ילוטייל", "המאצי", "לברק", "בס", "דניס", "בקלה", "אנשובי", "סרדין", "סרדינים",
  "לוקוס", "ברבוניה", "ברבוניות", "קוד", "מוסר", "בורי", "פלמידה", "אמנון", "פרגית", "שניצל", "פסטרמה", "שווארמה",
...
// 🔴 מה שפוסל **צמחוני** — בשר, דג ופירות ים, כולל כשהם רוטב. חלב וביצה אינם כאן.
// `inFamily` מחריג נשאים («רוטב», «ציר») בכוונה, כי מנה עם רוטב דגים אינה *מנת דג* —
// אבל היא בהחלט לא צמחונית. הבדיקה הזאת קוראת את המרכיבים בלי ההחרגה.
// נמדד על התפריט החי (14.9): «בנגקוק פאד תאי» עם **פיש סוס** הופיע בתשובה הנכונה
// ל«אילו מנות תמליץ ללקוח צמחוני».
const FLESH = ["דג", "דגים", "סלמון", "טונה", "ילוטייל", "המאצי", "לברק", "בס", "דניס", "בקלה", "אנשובי", "סרדין", "סרדינים",
  "לוקוס", "ברבוניה", "ברבוניות", "קוד", "מוסר", "בורי", "פלמידה", "אמנון", "פרגית", "שניצל", "פסטרמה", "שווארמה",
  "שרימפס", "שרימפ", "קלמארי", "תמנון", "סרטן", "צדפות", "מולים", "רכיכות", "טוביקו", "איקורה", "מסאגו", "ביצי דג",
  "אונאגי", "צלופח", "עוף", "כרעיים", "בקר", "אנטריקוט", "סינטה", "טלה", "כבש", "אסאדו", "קבב", "המבורגר", "נקניק",
  "בייקון", "חזיר", "פיש", "בונטיו", "דאשי"].map(norm);
const hasFleshWord = (text) => {
  const toks = norm(String(text || "")).split(/[\s,/]+/).filter(Boolean);
  return FLESH.some((k) => (EXACT_ONLY.has(k) ? toks.includes(k) : toks.some((t) => t.startsWith(k))));
};
// 🔴 היה `t.includes(k)` על כל השדה, ו-ANIMAL מכיל את «בס» בן שתי האותיות — כלומר
// «על **בס**יס וודקה», «**בס**פרינג רול» ו«**בס**גנון» סימנו מנה כלא-טבעונית. מנה
// שבתיאור שלה כתוב «מנה טבעונית» נפסלה בגלל הערת הגשה. הקובץ כבר מחזיק את הפתרון
// ל-FAMILIES (`EXACT_ONLY`) — כאן הוא פשוט לא הופעל. נתפס בסבב הבודקים, 14.9.
const hasAnimalWord = (text) => {
  const toks = norm(String(text || "")).split(/[\s,/]+/).filter(Boolean);
  return ANIMAL_KEYS.some((k) => (EXACT_ONLY.has(k) ? toks.includes(k) : toks.some((t) => t.startsWith(k))));
};
...
const FAMILIES = [
  ...
  ["עוף", ["עוף", "פרגית", "שניצל", "פסטרמה", "שווארמה"]],
  ...
  ["דג", ["דג", "דגים", "טונה", "סלמון", "ילוטייל", "המאצ", "לברק", "דניס", "מוסר", "בקלה", "אנשובי", "סרדינ", "פילה דג", "דג לבן",
    "לוקוס", "ברבוניה", "ברבוניות", "קוד", "בורי", "פלמידה", "אמנון"]],
].map(([label, words]) => [label, words.map(norm)]);
// «בס» ≠ בסיס · «דג» ≠ דגן · «קוד» ≠ קודם · «מוסר» ≠ מוסרים · «בורי» ≠ בוריטו
const EXACT_ONLY = new Set(["בס", "דג", "קוד", "מוסר", "בורי"].map(norm));
...
// 🔴 13.9: הגרסה הקודמת הייתה `/^(קרם|רוטב|…)\b/` ו**מעולם לא תפסה כלום** — שתי סיבות
// שכל אחת מהן לבדה מספיקה: `\b` של JS הוא ASCII ולכן אינו קיים אחרי אות עברית, והביטוי
// נבדק מול `norm(x)` שמקפל אותיות סופיות (קרם ⇒ קרמ, ציר בקר נשאר). התוצאה: «קיד נודלס»
// עם «רוטב צדפות» נספר כמנת פירות ים, «נאמס פרגית» עם «רוטב דגים» כמנת דג, ו«פסטה שימג׳י»
// עם «ציר בקר» כמנת בשר — כלומר מלצר שלא מנה אותן בשאלת-סט סומן כטועה.
const CARRIER = new RegExp(`^(${["קרם", "רוטב", "ממרח", "קציפת", "ציר", "אבקת", "שמן", "מרק", "שבבי", "פירורי"].map(norm).join("|")})(\\s|$)`);
...
export function buildSetQuestions(items, catLabel, opts = {}) {
  ...
  // 🔴 היה משפחות-מילים בלבד, והתפריטים החיים קוראים לחלבון בשמות שאינם באף רשימה
  // (ברבוניות · קוד · מוסר ים · שמות באנגלית). התוצאה: התשובה הנכונה ל«תמליץ ללקוח
  // צמחוני» בקטגוריית «From the Sea» הייתה **שתי מנות דג**. נתפס בסבב הבודקים, 14.9.
  // שכבה שנייה: דגל הכשרות «בשרי» ודגלי ההריון של בשר/דג נא הם נתון מתויג ולא ניחוש.
  const meatOrFish = (d) => inFamily(d, FAMILY_FISH)
    || inFamily(d, FAMILIES.find(([l]) => l === "פירות ים")[1])
    || inFamily(d, FAMILIES.find(([l]) => l === "בשר")[1])
    || inFamily(d, FAMILIES.find(([l]) => l === "עוף")[1])
    || (d.kashrut || []).some((k) => /בשרי/.test(k))
    || (d.pregnancy || []).some((x) => /דג נא|בשר נא/.test(x));
  // ⚠️ והשכבה שמונעת את הכשל שנשאר: בקטגוריה שרובה בשר או דג, «בטוחה» נקבעת מ**היעדר
  // ראיה** — בדיוק שם הזיהוי חלש ביותר. קטגוריה כזאת לא מקבלת את השאלה בכלל.
  // הסף 0.7 ולא 0.5: «ווק» הוא 2 מתוך 4 וזו שאלה לגיטימית לגמרי; «From the Sea» הוא
  // 10 מתוך 10. מדוד על שני התפריטים החיים, לא נבחר בהרגשה.
  // הרוטב נספר כאן גם כשהוא לא הופך את המנה למנת דג (`inFamily` מחריג נשאים בכוונה).
  const veggieBlocked = (d) => meatOrFish(d) || [...(d.ingredients || []), d.name].some((x) => hasFleshWord(x));
  const fleshShare = n ? dishes.filter(veggieBlocked).length / n : 0;
  const AUDIENCE = [
    ...
    { id: "veggie", who: "לקוח צמחוני", on: fleshShare < 0.7 && dishes.every((d) => (d.ingredients || []).length), ok: (d) => !veggieBlocked(d), why: (d) => (veggieBlocked(d) ? "יש בה בשר/דג" : "צמחונית") },
