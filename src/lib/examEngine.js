...
// כל אשכול נושא גם את המילה הלועזית-בעברית וגם את התרגום האנגלי (יותם, 1.9:
// «ספייסי לא שווה חריף, spicy לא שווה חריף») — מלצרים עונים בשלוש השפות האלה.
// הגשר עברית⇄לטינית של wMatch (שלד עיצורים) סוגר לבד את ספייסי⇄spicy, אז מילה
// אחת בכל כתב מספיקה לרוב.
const DESC_CLUSTERS = [
  ["מתוק", "מתקתק", "מתוקה", "מתקתקה", "מתיקות", "סוויט", "sweet"],
  ["מר", "מריר", "מרירה", "מרירות", "ביטר", "bitter"],
  ["חמוץ", "חמצמץ", "חמוצה", "חמצמצה", "חומציות", "חמיצות", "sour"],
  ["חריף", "חרפרף", "חריפה", "פיקנטי", "ספייסי", "spicy"],
  ["מרענן", "רענן", "מרעננת", "רעננה", "רעננות", "fresh"],
  ["קליל", "קל", "קלילה", "קלה", "לייט", "light"],
  ["חזק", "עוצמתי", "חזקה", "עוצמתית", "קשה", "כבד", "כבדה", "strong"],
  ["עדין", "עדינה", "חלש", "חלשה"],
  ["יבש", "יבשה", "dry"], ["מעושן", "מעושנת", "סמוקי", "smoky"],
  ["פירותי", "פירותית", "פרותי", "fruity"],
  ["פריך", "פריכה", "קריספי", "קריספית", "crispy"],
  ["טרופי", "טרופית", "tropical"], ["עשיר", "עשירה"], ["עשבוני", "עשבונית"],
...
export const GENERIC_ALT = {
  "ירקות": ["חסה", "עגבנייה", "עגבניה", "עגבניות", "בצל", "מלפפון", "גזר", "פלפל", "כרוב", "זיתים", "עלים", "תרד", "ברוקולי", "בצל ירוק", "צנונית", "סלט ירקות"],
  "רטבים": ["קטשופ", "מיונז", "איולי", "צ'ילי", "סויה", "טריאקי", "פונזו", "ברביקיו", "חרדל", "טחינה", "סחוג", "טרטר", "צזיקי", "רוטב"],
  "רוטב": ["קטשופ", "מיונז", "איולי", "צ'ילי", "סויה", "טריאקי", "פונזו", "ברביקיו", "חרדל", "טחינה", "סחוג", "טרטר"],
  "עשבי תיבול": ["פטרוזיליה", "כוסברה", "נענע", "שמיר", "בזיליקום", "אורגנו", "טימין", "רוזמרין", "עשבים", "תיבול"],
  "פירות": ["תות", "תותים", "מנגו", "אננס", "פסיפלורה", "אוכמניות", "פטל", "בננה", "תפוח", "אגס", "ענבים", "רימון"],
  "פטריות": ["שיטאקי", "שמפיניון", "פורטובלו", "פטריות יער", "שימג'י", "כמהין"],
...
export function generate(menu) {
  ...
  for (const d of menu) {
    ...
    if (ask.length >= 3 && !starGate && (!d.drink || d.drink === "יין" || d.drink === "סיגר")
      ...
      targets: ask.map(t => ({
        t, ctx: freeFor(d),
        ...(d.drink ? { alt: t === "כשר" && kosherIsr ? [...descAlts(t), kosherIsr, ...descAlts(kosherIsr)] : descAlts(t) } : (genericAlt(t).length ? { alt: genericAlt(t) } : {})),
  ...
  // הוא מילות תפריט). המלצה נכונה אחת = ציון מלא (minOk 1).
  {
    ...
    for (const [cat, drinks] of byCat) {
      ...
      // פתיחת בקבוקים (יותם, 31.8: «כמו בסטודיו — אלו בקבוקים צריך לפתוח
      // ואיך») — נגזר משורת «פתיחה:» שהנתונים של המסעדה נושאים: מבעבע נפתח
      // ביד עם כלוב התיל, יין רגיל בחולץ מלצרים. שאלה אחת לקטגוריה, ורק
      // כשהמסעדה באמת כתבה איך פותחים.
      if (kind === "יין") {
        const withOpen = drinks.find((d2) => /פתיחה:/.test(d2.desc || ""));
        if (withOpen) {
          // עד סוף התיאור, לא עד הנקודה הראשונה — פרוטוקול ההגשה (הצגה, טעימה,
          // מזיגה) יושב אחרי הנקודה, ובלעדיו הוא נספר כהמצאה. נתפס בשער האמת.
          const sent = (withOpen.desc.match(/פתיחה:.*$/) || [""])[0];
          const spark = /כלוב|תיל/.test(sent);
          out.push({
            sit: "drinkrec", dish: `${cat} · פתיחה`, cat, k: "recall", secs: 60,
            ask: spark
              ? `אורח הזמין בקבוק מ${cat}. איך פותחים אותו ליד השולחן?`
              : `אורח הזמין בקבוק מ${cat}. עם מה פותחים אותו, ומה הסדר מול האורח?`,
            targets: (() => spark
              ? [{ t: "ביד, עם כלוב התיל — לא בחולץ", must: ["ביד"], alt: ["כלוב", "תיל", "יד"] },
                 { t: "פתיחה שקטה, טעימה למזמין ואז מזיגה", alt: ["שקטה", "טעימה", "מזיגה", "הצגה"] }]
              : [{ t: "חולץ מלצרים", must: ["חולץ"] },
                 { t: "הצגה, פתיחה, טעימה למזמין ואז מזיגה", alt: ["הצגה", "טעימה", "מזיגה", "למזמין"] }])(),
            // כל מילה בניסוח המטרות חופשית — נגזר, לא רשימה ידנית: «ואז» של
            // ניסוח היעד נספר כהמצאה כשחסר כאן (נתפס בשער האמת, פעמיים).
            get free() {
              return [...toks(sent), "בקבוק", "יין", "פותחים", "לפתוח",
                ...this.targets.flatMap((t) => toks(t.t))];
            },
            minOk: 1, maxInv: 1,
          });
        }
      ...
      if (kind !== "יין") {
        ...
        out.push({
          ...
          ask: mixed
            ? `אורח שואל אילו ${cat} יש. מה תציע לו?`
            : `אורח שואל ${fem} ${kind} יש. מה תציע לו?`,
      ...
      // ההמלצה מוגבלת למה שבאמת מוגש בבקבוק; להמליץ על בקבוק שאין = wrongFar.
      {
        ...
        if (bottled.length >= 2) {
          const KINDS_RE2 = [/וודקה/, /וויסקי|ויסקי/, /טקילה/, /ג['׳]ין/, /ערק|אוזו/, /קוניאק|ברנדי/, /ליקר/, /רום/, /אפריטיף|ורמוט/];
          const mixed2 = KINDS_RE2.filter((r) => r.test(cat)).length >= 2;
          const n = Math.min(2, bottled.length);
          out.push({
            sit: "drinkrec", dish: `${cat} · בקבוק לשולחן`, cat, k: "recall", secs: 60,
            ask: `אורח רוצה בקבוק ${mixed2 ? `מ${cat}` : kind} לשולחן. על ${n === 1 ? "איזה בקבוק" : `${n} בקבוקים`} תוכל להמליץ?`,
