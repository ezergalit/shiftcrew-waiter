...
MainApp.jsx (src/screens/MainApp.jsx):
  ...
  **מחיר הוסר מכל המשחקים (2026-08-11, משוב משתמש)**: `it.price` עדיין קיים במודל
  ...
  - **מבחן קטגוריה** (`CategoryExam`, מצב `exam`, חדש 2026-08-11): שלב הסיום של קטגוריה.
    מציג **רק שם המנה**, והמלצר כותב בטקסט חופשי מה יש בה. נבדק מול `ingredients`+`allergens`
    האמיתיים (מה שבעל המסעדה כבר הזין) — **ללא AI, ללא קריאת API, עלות אפס**. ציון =
    60% כיסוי מרכיבים + 40% כיסוי אלרגיות; מנה בלי אלרגיות → 100% מהמרכיבים. **פספוס
    אלרגיה מציג אזהרה אדומה בולטת** וחוסם ציון גבוה גם אם כל המרכיבים נכונים (60% במקרה
    כזה) — כי בשירות אמיתי זה הדבר המסוכן. 4 מנות למבחן, ציון סופי = ממוצע, מעבר ב-70%.
    ממופה ל-mastery 1-5 כמו כל מצב אובייקטיבי אחר, כך שתוצאת מבחן מזיזה את אחוז ההצלחה
    ומשתקפת לבעלים. **נעול עד שהקטגוריה ב-50%+** (אחרת זה סתם קיר של תיבות ריקות).
    ⚠️ **החלטה ארכיטקטונית מפורשת של המשתמש**: הגרסה הראשונה שתוכננה שלחה כל תשובה
    ל-Claude לבדיקה סמנטית. זה נפסל כי זו קריאת API אחת לכל תשובה × כל מלצר × לנצח.
    אם בעתיד רוצים בכל זאת שכבת AI — **המקום הנכון הוא אפליקציית הבעלים, פעם אחת בבניית
    התפריט** (לייצר "מפתח בדיקה" מועשר לכל מנה שנשמר ב-DB ומשרת את כל המלצרים), לא
    בזמן אמת בצד המלצר.

  **התאמת טקסט עברי (`normWords`/`variants`/`mentions`)**: המבחן חייב לסבול איך שאנשים
  באמת כותבים. מנרמל ניקוד, אותיות סופיות (ם→מ), וסימני פיסוק; ומתאים אם מילה כלשהי
  מהמונח מופיעה בתשובה ("כמהין" מספיק ל"כמהין שחור"). **בכוונה סלחני** — המטרה לתגמל
  הבנה של המנה, לא שינון הניסוח של הבעלים.
  ⚠️ **לקח מבאג אמיתי (נתפס בבדיקת עיקריות, 2026-08-11)**: אות שימוש מובילה (ו/ב/ל/ה/מ/ש/כ)
  היא *דו-משמעית* — "ובזיליקום" זה ו'+בזיליקום, אבל "בזיליקום" עצמה מתחילה ב-ב' אמיתית.
  הגרסה הראשונה הסירה את האות **ללא תנאי משני הצדדים**, מה שהפך כל מרכיב שמתחיל באות
  כזו (בזיליקום, שום, שמנת...) לבלתי-ניתן-להתאמה בחלק מהמקרים. התיקון: `variants(w)`
  מחזירה את **שתי** הקריאות (עם ובלי האות) ומספיקה התאמה של כל צימוד. אם נוגעים שוב
  בלוגיקת ההתאמה — לבדוק עם מילים שמתחילות באותיות שימוש משני הצדדים.
