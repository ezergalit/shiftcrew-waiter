...
MainApp.jsx (src/screens/MainApp.jsx):
  ...
  **התאמת טקסט עברי (`normWords`/`stem`/`mentions`)**: המבחן חייב לסבול איך שאנשים באמת
  כותבים. מנרמל ניקוד, אותיות סופיות (ם→מ), וסימני פיסוק; מסיר אות שימוש מובילה
  (ו/ב/ל/ה/מ/ש/כ) כשנשארת מילה אמיתית ("וקרקרים"→"קרקרים", אבל "מלח" לא נוגעים); ומתאים
  אם מילה כלשהי מהמונח מופיעה בתשובה ("כמהין" מספיק ל"כמהין שחור"). **בכוונה סלחני** —
  המטרה לתגמל הבנה של המנה, לא שינון הניסוח של הבעלים.
