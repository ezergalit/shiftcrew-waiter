...
אפליקציית web לצוות המסעדה — כניסה בקוד צוות (`LEARN`) + שם (ללא סיסמה), לימוד תפריט
...
main.jsx → App.jsx
              ...
              │
TeamLogin.jsx:
  מזין team_code (LEARN) + שם → מחפש restaurant לפי team_code →
  יוצר שורת team_members חדשה (בכל כניסה — ר' "TODO" למטה) → שומר session
...
MainApp.jsx (src/screens/MainApp.jsx):
  ...
  - Matching (נבנה מחדש 2026-08-10, בסגנון Quizlet): גריד 3 עמודות, 6 מנות עם
    מחירים שונים זה מזה (לא בוחר שתי מנות באותו מחיר — ר' TODO) × 2 tiles (שם+מחיר)
...
- **TeamLogin יוצר team_member חדש בכל כניסה** — אין דה-דופליקציה לפי שם. אם אותו אדם
  נכנס פעמיים עם אותו שם, ייווצרו שני `team_members` שונים עם שני מסלולי leaderboard/progress
  נפרדים (ה-session נשמר ב-localStorage אז בפועל זה קורה רק פעם אחת למכשיר, אבל שווה
  לשים לב אם המשתמש מדווח על "איבדתי את ההתקדמות שלי").
